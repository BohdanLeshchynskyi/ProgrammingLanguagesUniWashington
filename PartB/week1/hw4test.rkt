...
(define tests
  (test-suite
   ...
   (check-equal? (stream-for-n-steps ones 2) (list 1 1) "stream-for-n-steps test on ones")

   (check-equal? (stream-for-n-steps nats 5) (list 1 2 3 4 5) "stream-for-n-steps test on nats")
   ...
   (check-equal? (stream-for-n-steps (stream-add-zero ones) 1) (list (cons 0 1)) "stream-add-zero test ones")

   (check-equal? (stream-for-n-steps (stream-add-zero nats) 5) (list (cons 0 1) (cons 0 2) (cons 0 3) (cons 0 4) (cons 0 5)) "stream-add-zero test nats")
   ...
   (check-equal? (vector-assoc 0 (vector (cons 2 1) (cons 3 1) (cons 4 1) (cons 5 1))) #f "vector-assoc test2")

   (check-equal? (vector-assoc 4 (vector (cons 2 1) 1 (cons 4 1) "mark")) (cons 4 1) "vector-assoc test3")
